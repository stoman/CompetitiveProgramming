...
data class Board(val numbers: List<List<Int>>) {
  fun isCompleted(draw: List<Int>): Boolean = numbers.any { draw.containsAll(it) } || numbers.transpose().any { draw.containsAll(it) }
}
...
fun main() {
  ...
  for(drawIndex in draw.indices) {
    ...
    for(board in boards) {
      if(board.isCompleted(allDraws)) {
        println(allDraws.last() * board.numbers.flatten().filterNot { it in allDraws }.sum())
        return
      }
    }
  }
