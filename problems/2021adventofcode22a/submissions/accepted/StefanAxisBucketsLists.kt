...
private data class Instruction2(
  val on: Boolean,
  val xMin: Int,
  val xMax: Int,
  val yMin: Int,
...
private fun List<Int>.distToNext(i: Int): Long = if (get(i) !in -50..50) 0 else (get(i + 1) - get(i)).toLong()
  ...
  val instructions = buildList {
    while (s.hasNext()) {
      add(
        Instruction2(
          s.next() == "on",
          s.nextInt().coerceAtLeast(-51),
          s.nextInt().coerceAtMost(51),
          s.nextInt().coerceAtLeast(-51),
          s.nextInt().coerceAtMost(51),
          s.nextInt().coerceAtLeast(-51),
          s.nextInt().coerceAtMost(51)
