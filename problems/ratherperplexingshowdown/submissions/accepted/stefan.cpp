		...
		if(!ok) {
			...
			for(int i = n-1; i >= 0; i--) {
				...
				for(int j = 0; j < sum[i+1]; j++) {
					...
					else if(ret[i+1][j] == 'P') ret[i] += "PR";
					else if(ret[i+1][j] == 'S') ret[i] += "PS";
				}		
			}
			//sort
			for(int i = 1; i < n; i++) {
				for(int j = 0; j < sum[0]; j+= 2*sum[n-i]) {
					if(ret[0].substr(j, sum[n-i]) > ret[0].substr(j+sum[n-i], sum[n-i])) {
						string tmp = ret[0].substr(j+sum[n-i], sum[n-i]);
						ret[0].replace(j+sum[n-i], sum[n-i], ret[0].substr(j, sum[n-i]));
						ret[0].replace(j, sum[n-i], tmp);
					}
				}
			}
